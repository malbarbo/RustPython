...
impl CompilationSource {
    ...
    fn compile_dir(
        ...
        // Load the freeze allowlist once at the top-level Lib/ call.
        // The allowlist contains dotted module names (e.g. "re._compiler")
        // generated by: scripts/find_stdlib_deps.py
        let allowlist: Option<HashSet<&str>> = if parent.is_empty()
            && path.file_name().is_some_and(|n| n == "Lib")
        {
            let list_path = path.join("freeze_allowlist.txt");
            if list_path.is_file() {
                let content: &'static str =
                    Box::leak(fs::read_to_string(&list_path).unwrap().into_boxed_str());
                Some(content.lines().filter(|l| !l.is_empty()).collect())
            } else {
                None
            }
        } else {
        ...
        for path in paths {
            ...
            if path.is_dir() {
                ...
                if let Some(allowed) = allowlist {
                    ...
                    if !allowed.contains(child_mod.as_str())
                        && !allowed.iter().any(|m| m.starts_with(&prefix_dot))
                    {
                        continue;
                    }
                }
                ...
                if let Some(allowed) = allowlist {
                    ...
                    if !module_name.is_empty() && !allowed.contains(module_name.as_str()) {
                        continue;
                    }
                }
